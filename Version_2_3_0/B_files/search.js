...
function DoSearch() {
	...
	for (Book in Bible) {     //Books
		if (SearchFor.test(Bible[Book].toString()) > 0) {     //Book
			window.status = NoMatches + ":  " + Book;
			for (var C = 1; C < Bible[Book].length; C++) {     //Chaps
				if (SearchFor.test(Bible[Book][C].toString()) > 0) {
					for (var V = 0; V < Bible[Book][C].length; V++) {     //Verse
						if (SearchFor.test(Bible[Book][C][V].toString()) > 0) {
							//MyFound.push(Book+" : "+Bible[Book][C][V].toString());
							//alert(SearchFor.test(Bible[Book][C][V].toString()));
							MyFound.push(new BibleRef(Book, C, V, 1, SearchFor.test(Bible[Book][C][V].toString())));
							NoMatches++;

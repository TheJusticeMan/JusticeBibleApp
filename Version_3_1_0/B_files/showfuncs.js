...
function fixItal(_string) {
	return (_string.replace(/\[/g, "<em>").replace(/\]/g, "</em>").replace(/LORD/g, "<strong class=LORDCAPS>Lord</strong>"))
}
...
class HistoryPlace {
	...
	HistoryText() {
		return " (" + this.Book + "  " + (this.Chap) + ":" + (this.Verse + 1) + ", KJV)";
...
function ShowRandomVerse() {
	closeNav();
	var MBook = booksOfTheBible[Math.floor(Math.random() * 66)];
	var MChap = Math.floor(Math.random() * (Bible[MBook].length - 1)) + 1;
	var MVerse = Math.floor(Math.random() * Bible[MBook][MChap].length);
	History.push(new HistoryPlace(MBook, MChap, MVerse));
}
...
function ShowMenu() {
	...
	s += "<a href='#' onclick='return ShowHistory();'>Show History</a>";
	s += "<a href='#' onclick='return ShowRandomVerse();'>Random verse</a>";
